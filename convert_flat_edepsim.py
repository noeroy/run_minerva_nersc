...
def main(args,option_type):
    ...
    struct_meta.processing = "Picorun4.1"
    ...
    for entry in range (max_entry):
        ...
        for key, seg in segment:
            ...
            # if (key[:8] != "Detector"):
            #     continue
            # if ((key[:20] != 'DetectorPlanelvScint') and ((key[:20] != "DetectorPlanelvStrip")) ):
            #     continue
            
            # if (((key[:15] == "DetectorPlanelv")) or ((key[:22] == "DetectorPlanelvTracker")) ):    
                # continue
            # my_volume = mydict[key]
            # my_volume = int(my_volume)
            ...
            for hit in seg:
                ...
                m_StartX.push_back(hit.GetStart().X())
                m_StopX.push_back(hit.GetStop().X())
